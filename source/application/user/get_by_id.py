...
class GetUserSchemaById(Interactor[str, UserSchema]):
    ...
    async def __call__(self, max_id: str) -> UserSchema | None:
        try:
            async with self.uow:
                user: UserSchema = await self.repository.get_schema_by_max_id(
                    max_id
                )
                return user
...
class GetUserById(Interactor[str, User]):
    ...
    async def __call__(self, max_id: str) -> User | None:
        try:
            async with self.uow:
                user = await self.repository.get_model_by_max_id(
                    max_id
                )
                return user
        except Exception as exc:
